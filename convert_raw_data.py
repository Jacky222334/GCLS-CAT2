...
# Einlesen der Datei
df = pd.read_excel('data/Raqw_data_strings.xlsx')
...
output_file = 'data/raw_data_numeric_recoded.xlsx'
...
# Überprüfung der umgepolten Items
print("\nMittelwerte der umgepolten Items:")
for item in items_to_reverse:
    print(f"{item}: {df[item].mean():.2f}")

print(f"\nDatei wurde gespeichert als: {output_file}")

# Prüfen auf fehlende Werte oder nicht umgewandelte Strings
na_check = df.isna().sum()
if na_check.any():
    print("\nAchtung! Fehlende oder nicht umgewandelte Werte gefunden:")
